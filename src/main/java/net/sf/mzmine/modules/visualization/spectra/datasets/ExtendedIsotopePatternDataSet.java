...
/**
 * 
 * Dataset that can contain an ExtendedIsotopePattern and convert it so
 * it can be accessed by an bar renderer. This also contains the description 
 * of the datapoints so they can be displayed as a tooltip in the preview chart.
 * The tooltips can be generated by SpectraToolTipGenerator. Furthermore, a
 * minimum intensity is specified and the peaks are sorted into two different
 * XYSeries to allow a color differentiation if the relative intensities are
...
public class ExtendedIsotopePatternDataSet extends XYSeriesCollection {
   ...
   */
  public ExtendedIsotopePatternDataSet(ExtendedIsotopePattern pattern, double minIntensity, double width)
  {
...
//  the next two methods are not needed yet but might come in handy in the future
   ...
   */
  @Override public double getStartXValue(int series, int item) {
   ...
   * By redefining this, the pattern is displayed as an actual bar instead of a line
   */
  @Override public double getEndXValue(int series, int item) {
